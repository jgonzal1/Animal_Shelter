...
-- Species
CREATE TABLE Reference.Species (Species VARCHAR(10) NOT NULL PRIMARY KEY);
...
VALUES ('Dog'),
  ...
  ('Ferret'),
  ('Raccoon');
-- And a couple species for which we won't have any animals
-- Species vital signs normal ranges
CREATE TABLE Reference.Species_Vital_Signs_Ranges (
  Species VARCHAR(10) NOT NULL PRIMARY KEY REFERENCES Reference.Species (Species),
...
CREATE TABLE Reference.Breeds (
  Species VARCHAR(10) NOT NULL REFERENCES Reference.Species (Species),
...
SELECT 'Dog' AS Species,
  LEFT(Breeds.Breed, 1) + RIGHT(LOWER(Breeds.Breed), LEN(Breeds.Breed) - 1) AS Breed,
  Breeds.URL
FROM (
...
-- Colors
CREATE TABLE Reference.Colors (Color VARCHAR(10) NOT NULL PRIMARY KEY);
-- Populate colors
...
CREATE TABLE Reference.Vaccines (Vaccine VARCHAR(50) NOT NULL PRIMARY KEY);
...
CREATE TABLE Reference.Species_Vaccines (
  Species VARCHAR(10) NOT NULL REFERENCES Reference.Species (Species),
  Vaccine VARCHAR(50) NOT NULL REFERENCES Reference.Vaccines (Vaccine),
...
CREATE TABLE Reference.Cities (
  State VARCHAR(20) NOT NULL REFERENCES Reference.States (State),
...
CREATE TABLE Reference.City_Zip_Codes (
  ...
  City VARCHAR(30) NOT NULL,
  CONSTRAINT FK_City_Zip_Codes__Cities FOREIGN KEY (State, City) REFERENCES Reference.Cities (State, City),
...
VALUES (1, 'Main'),
  ...
  (50, 'Madison');
-- Integers
CREATE TABLE Reference.Integers (Number INT NOT NULL PRIMARY KEY);
...
WITH Calendar_Holidays AS (
  ...
  FROM Reference.Calendar AS C
    INNER JOIN Reference.Federal_Holidays_Fixed AS FHF ON C.Month = FHF.Month
...
WITH Calendar_Holidays AS (
  SELECT C.Date,
    ...
    FHF.Holiday
  FROM Reference.Calendar AS C
    INNER JOIN Reference.Federal_Holidays_Floating AS FHF ON C.Month = FHF.Month
